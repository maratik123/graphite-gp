...
- **The delegation lifecycle has FOUR phases; the bullets either side of this one cover only the endpoints, and the middle is where failures land.** **(2) Hand-off state** — leave the index **CLEAN** before delegating to a subagent that commits. `git commit` captures the **whole index**, not just what the delegate's own `git add` added, so anything you pre-staged (a `git mv`, a half-finished edit) lands in its first commit. Commit your own staged work first, or `git restore --staged <path>` to keep it working-tree-only. **(3) While it runs** — a delegate ending its turn to wait on a long job is **waiting**, not stuck. Do NOT start a parallel investigation of the same question: it duplicates the work and steals CPU from the very job you are impatient about. Verify what it is actually doing (`ps`, its committed output) before judging; if it lacks information you already have, send it that information plus an explicit decision rule; take over only after stopping it, and say so. **(4) Take-over** — if you revert and re-author its work by hand, treat its touched-file list as a **checklist**; the sub-edits (propagations, sync-group siblings) are exactly what a manual redo drops. Run the § *Propagation Rule* grep before commit, unconditionally.
- **A subagent's RETURN SUMMARY is a claim, not a record — reconcile it against the durable artifact before trusting it.** After any group/subagent returns, verify every gate / PASS / "I did X" assertion in its summary against the durable record (the `.progress.md`, the commit body, `git log` / `git diff`, the file itself); trust the durable record when they disagree, and close the gap in-thread (e.g. run `image-check` yourself if the progress file shows it was skipped). Applies with extra force to a delegate that also writes shared files — a `code-writer`'s "I appended a learning" and its "golden verified by image-check → PASS" have each proven false against the durable record. Treat a delegate's stated REASON for skipping a gate as suspect too, especially when its other observed behaviour contradicts it.
...
> Per-skill instances: `/task` Step 10, `/pr-commented` Step 5, `/pr-ci-failed` Step 5, `/main-ci-failed` Step 5, `/bugfix` Step 6. This AXIOM names them as one workspace rule so the next surface without its own step still falls under it — the enumeration is a list of *named* instances, **never** a list of the only covered surfaces. An unnamed surface shipping executable code (a hook body, a script) is covered; `/improve` is the standing example.
>
> **Carve-out — `/reflect` is exempt.** Not on cost grounds but **structural** ones: `/reflect`'s product is `learnings.md` entries, and every consumer of that surface is already contractually obliged to re-verify — `.claude/agents/self-improve.md` § Step 3's CANDIDATE-truth AXIOM plus its mandatory `Claims re-verified` line on the `/improve` path, and `.claude/agents/design.md`'s `[measured:]` requirement on the filed-issue → `/task` path. Verify a reflection entry's claims **inline while writing it**. Generalises: **verify at the point of CONSUMPTION, not the point of RECORDING** — before adding any gate, ask where the claim is already checked downstream.
>
...
- **Miri gate — a red Miri is a REGRESSION TO FIX, but does not currently block merge** ([#76](https://github.com/maratik123/graphite-gp/issues/76), resolved 2026-07-18): the `miri-pass` aggregator produces the `Miri` context (mirrors `build-pass`; a path-filtered `skipped` Miri still passes it) and the raw job carries no `continue-on-error`, so a red Miri reddens the job → the workflow run and **is** visible in `gh run list`. **`Miri` is deliberately NOT among `main-protection`'s required contexts** — verified 2026-07-25, the required set is `Build`, `Clippy`, `Docs`, `Format`, `Test`. The gate is held out of branch protection **while its wall-clock is long**, pending [#134](https://github.com/maratik123/graphite-gp/issues/134) for `gp-gen` plus further per-crate tuning; restore it to the required set once those land. Miri **aborts on the FIRST unsupported operation** and cargo's fail-fast then drops every phase queued behind it (the offending crate's whole test binary + the doc-test phase), so gate every test that can **abort** Miri — OR is a zero-production-UB-signal **cost** test — with `#[cfg_attr(miri, ignore = "<why>")]` **in the same commit**; per-test, **never** a crate-level `--exclude` (that also drops the crate's Miri-clean tests) — save the one sanctioned `gp-gen` cost carve-out (#134) noted below. The trigger is **"aborts (or costs) under Miri", NOT "is FFI"**, and the reason must describe **that test's own** cause — never a sibling's (a wrong reason is a false justification for a different failure). Reproduce with the **workspace** command CI runs (`MIRIFLAGS=-Zmiri-tree-borrows cargo miri test --workspace --exclude gp-gen`; locally add `+nightly`), never a narrower `-p` run — that governs the **shape** of a local Miri run, not its **frequency**: local Miri is exception-only (strong named reason), never routine Step-8 verification (see [`ai-docs/miri-gate.md`](ai-docs/miri-gate.md)). **`gp-gen` is excluded from the Miri gate** — a *temporary cost* carve-out (its multi-seed generator sweeps regressed Miri wall-clock past 1h), tracked by [#134](https://github.com/maratik123/graphite-gp/issues/134) and to be re-enabled via gated seed sweeps once it resolves; this is the **one sanctioned crate-level `--exclude`** (justified by cost, not correctness — `gp-gen` is pure-integer and Miri-clean). Treat a red Miri as a **regression to fix** — green it, or gate the aborting test, before merge; the discipline is unchanged by the fact that branch protection does not currently enforce it. **Full gate mechanics + the two mechanical `gp-render` triggers (Context/painter tests; zero-production-UB-signal token/helper tests) live in [`ai-docs/miri-gate.md`](ai-docs/miri-gate.md).** Check Miri after any PR adding a new dependency class.
- Unit tests in the same file under a `#[cfg(test)]` module. Integration tests in `tests/`.
- Use `rstest` for parameterized tests when useful; `mockall` for mocking traits; `pretty_assertions` encouraged for diffs. `proptest` (gp-core dev-dep) is for **differential** properties — pinning a rewrite to the implementation it replaced. Bound the *input space by what the oracle costs*, not by the type's range: a kept-verbatim `O(bbox)` reference makes a free `±10_000` pair scan 4·10⁸ cells per case. Precedent: `geom/supercover.rs` § `supercover_equivalence`.
- Assert with `assert_eq!` / `assert_matches!`. **`assert_matches!` formats the scrutinee with `{:?}` on mismatch, so its type MUST impl `Debug`** (`Result` needs both `T` + `E`; `Box<dyn Trait>` needs a `Debug` supertrait) — `assert!(matches!(...))` imposes no such bound. If the scrutinee is non-`Debug`, leave `assert!(matches!(...))` as-is — do NOT add a production `#[derive(Debug)]` to satisfy a test-only assertion. Counting `assert!(matches!)` sites for a migration: the multi-line message form is invisible to single-line `rg 'assert!\(matches!'` — use `rg -U`. That is one instance of a general rule — a search miss on a construct that SHOULD exist is a **search-method failure first, code-absence second** — whose causes and fixes live in [`.claude/rules/ast-index.md` → Negative results are NOT evidence](.claude/rules/ast-index.md#negative-results-are-not-evidence).
