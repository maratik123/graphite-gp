...
| `crates/gen/src/phase1.rs:258` | `RaceDir::iter().choose(rng).expect("strum's generated iter always correctly generates size_hint and nth")` | `IteratorRandom::choose` relies on correct implementation of `nth` and `size_hint`. Both provided by strum's generated `RaceDirIter` |
