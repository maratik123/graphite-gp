...
- **Next implementation step:** continue block 3a — the signed lap counter, crash resolution, and car-collision resolution in `crates/core/src/sim.rs`, then the passability oracle. (`sim::step`, `supercover`, and the corridor-graph helpers — `flood_fill` / `component_count` / `bounded_complement_components`, geodesic BFS, `walls_from_boundary` — are done; the `TrackArtifact` contract is finalized, so `LapCounter::register_move` can now read the `TimingGate` (`separates` + `forward`) off `StartFinish` for its half-open signed-crossing test; `resolve_collisions` can build nearest-free placement on `CorridorScratch::geodesic_bfs`, and `gen`'s Ф4 static-validation on the component/complement helpers.)
