...
- **Block 1 (`gp-gen`) — in progress:** the grouped seeded-RNG config (`gp_core::rng::Seeds`, #49/#50) plus Ф1 coarse-block ring (#24), Ф2 rasterize-to-`D` (#25), Ф3 start/finish + accel zone + start grid + timing gate (#26), and Ф4 static validation (#27 — the four static checks emitting typed `Issue`s [`Disconnected`/`BadTopology`/`Narrow`/`NarrowSf`/`LostHairpin`] for the future Ф6 repair phase, plus reusable integer distance-transform + medial-axis primitives in `gp-core` `geom`) have landed; next unit is **Ф5–Ф7** (oracle / local repair / output).
