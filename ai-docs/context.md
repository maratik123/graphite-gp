...
- **Next implementation step:** continue block 3a — crash resolution and car-collision resolution in `crates/core/src/sim.rs`, then the passability oracle. (`sim::step`, `supercover`, `LapCounter::register_move` (issue #8), and the corridor-graph helpers — `flood_fill` / `component_count` / `bounded_complement_components`, geodesic BFS, `walls_from_boundary` — are done; `resolve_collisions` can build nearest-free placement on `CorridorScratch::geodesic_bfs`, and `gen`'s Ф4 static-validation on the component/complement helpers. The deferred tick/turn driver loop (ordering `legal_move` → `step` → `register_move` → collision/crash resolution, then reading `laps()`/`raw()` for the win check) lands with the crash/collision tasks.)
