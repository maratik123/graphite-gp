...
- **The binding-constraint file for anything you specify.** Before writing "component X does Y", read the file that **CONSTRAINS** X — not a file showing X is *capable* of Y:
  ...
  - **`ai-docs/panic-index.md` + `ai-docs/context.md`** — gp-core targets **zero production panics** and the index currently has **no `crates/core/` row**. It has been non-empty exactly once: PR [#171](https://github.com/maratik123/graphite-gp/pull/171)'s `supercover` interval solver added two `i32::try_from(..).expect(..)` bounds, and the revert removed them — so read the index for the live row set rather than trusting either state from memory. Treat any proposed `.expect` / `unwrap` / `panic!` / panicking index in `sim`/`geom` as a **red flag against that invariant**: prefer a *total* form (`checked_*` / `saturating_*` / `try_from(..).unwrap_or(sentinel)` / `Option`) giving defined behaviour in the impossible case, mirroring the #48 integer-safety posture. Adding a gp-core panic-index entry must be a deliberate, justified exception — never an incidental `.expect` with a `# Panics` doc section.
