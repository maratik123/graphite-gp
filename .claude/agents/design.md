...
- **The binding-constraint file for anything you specify.** Before writing "component X does Y", read the file that **CONSTRAINS** X — not a file showing X is *capable* of Y:
  ...
  - **`ai-docs/panic-index.md` + `ai-docs/context.md`** — gp-core targets **zero production panics**. The index is **no longer empty for gp-core**: PR [#171](https://github.com/maratik123/graphite-gp/pull/171) added two `i32::try_from(..).expect(..)` sites in `geom/mod.rs`'s `supercover` inner-bound helper (hand-authored outside `/task`). Read the index for the live row set rather than assuming either state. Treat any proposed `.expect` / `unwrap` / `panic!` / panicking index in `sim`/`geom` as a **red flag against that invariant**: prefer a *total* form (`checked_*` / `saturating_*` / `try_from(..).unwrap_or(sentinel)` / `Option`) giving defined behaviour in the impossible case, mirroring the #48 integer-safety posture. Adding a gp-core panic-index entry must be a deliberate, justified exception — never an incidental `.expect` with a `# Panics` doc section — and those two existing rows are precedent for *what already leaked*, never a licence for the next one.
...
- **Text-bearing golden tests specify the measured threshold AT DESIGN TIME.** When § Test Design specs a golden that renders text (glyphs, labels, numerals, icons), the design MUST pin `.threshold(1.0).failed_pixel_count_threshold(0)` — NEVER `threshold(0.0)`, and NEVER a "mint at 0.0, bump if CI reds" strategy; the content class is settled and 0.0 schedules a wasted red-CI round. Reserve exact `threshold(0.0)` for flat / byte-stable content. Specifying 0.0 for a text golden is the defect self-review would REJECT — catch it a phase earlier, here. See [`ai-docs/code-style.md` → Golden-image thresholds](../../ai-docs/code-style.md#golden-image-thresholds).
- **Shared-boundary fill/stroke consistency (render designs).** When one layer fills a region whose boundary another layer strokes (or vice-versa), the design MUST require both to be built from the **same boundary geometry** — a per-cell square fill under a smoothed (Chaikin) stroke disagrees at every corner *by construction* (staircase notches / colour bleed) and passes every automated gate (`image-check`, exact compare, `self-review`, CI). Flag it at design time and prescribe a shared-mesh fill (triangulate once, tint/clip per-cell). A § Risks note ("convex-corner bleed") is **not** a substitute — an occurrence shipped *with* that note. See [`ai-docs/code-style.md` → Shared-boundary fill/stroke consistency](../../ai-docs/code-style.md#shared-boundary-fillstroke-consistency).
- **Golden setup fidelity (render designs).** The design MUST (1) reuse an already-built, owner-approved domain fixture (a `scene_*`/gallery fixture shared via a `pub(crate)` `#[cfg(test)]` move) rather than hand-rolling one from the nearest unit-test fixture, and (2) require a composition-root/full-screen widget to paint its own background + use its own palette tokens, and require the golden harness to render under the binary's real conditions rather than forcing a theme/size the binary never sets (`.with_theme(...)`, window size, visuals). Both failure modes pass every automated gate (`image-check`, exact/measured compare, `self-review`, CI) while the real output is broken. See [`ai-docs/code-style.md` → Golden setup fidelity](../../ai-docs/code-style.md#golden-setup-fidelity--fixture--harness-must-match-the-real-thing).
- **Never spec a routine local Miri run as per-subtask verification.** Miri is CI's job. A design asking for a local workspace Miri run at Step 8 buys a multi-ten-minute interpreter rebuild for a signal CI already produces; spec the per-test `#[cfg_attr(miri, ignore = "<why>")]` gate instead. Local Miri belongs in a design only with a named diagnostic reason (an already-red run to diagnose, or an abort-vs-cost decision to settle). See [`ai-docs/miri-gate.md`](../../ai-docs/miri-gate.md).
