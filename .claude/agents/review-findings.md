...
- Any test that would pass even if the production code were deleted (cosmetic test)? **Sub-case — vacuous guard-clause test:** a "no false positive" / `is_empty()` / "reports nothing" assertion whose fixture never satisfies the guarded clause's *pre-condition*, so the clause it names is never exercised (the enclosing function still runs — only one clause is dead). Verify by mutation: delete that clause from production; if the test still passes, the fixture must be rebuilt so the pre-condition IS met at some cell while the clause is what rejects. (`self-review.md` § Patterns 2.)
