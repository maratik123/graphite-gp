...
**Why the parent dispatches — a MAY rule, not a CAN rule.** `Agent` **IS** callable from this Subagent class here (probed 2026-07-17). The contract is nonetheless the parent's, because the parent owns the **user-facing report** — and **a capability grant is evidence about CAN, never about MAY**. Do not re-derive it from your tool list. Provenance, the expired-not-fabricated prior claim, and the async-dispatch gotcha: [`ai-docs/improve-eval-contract.md`](../../ai-docs/improve-eval-contract.md).

**Step 6 handoff — pause-and-surface protocol** (the parent thread, NOT the subagent, dispatches the reproducers):

1. **Assemble, do not dispatch.** You have `Agent`; **do not use it for Step 6** — the restriction is contractual, per the paragraph above. Do NOT substitute a degraded same-context path: no `Bash`-shelled invocation, no `TaskCreate`-then-`TaskOutput` polling, no in-memory close-read. Each is forbidden **on its own merits** — none runs the reproducer in a *clean context*, which is the entire point. If you believe the contract is wrong, **say so in your report**; do not resolve it by acting. (Authority + the quartzite precedent: [`ai-docs/improve-eval-contract.md`](../../ai-docs/improve-eval-contract.md).)
