...
**Why the parent dispatches — a MAY rule, not a CAN rule.** `Agent` **IS** present and callable from this Subagent class: probed 2026-07-17, a live `Agent(subagent_type: "general-purpose", …)` dispatch from inside a `self-improve` spawn launched and returned `PROBE_OK` intact. The prior *"structurally unfulfillable — the runtime tool exposure genuinely lacks `Agent`"* claim is **FALSE as of 2026-07-17** and has been removed. It cited two 2026-05-15 `learnings.md` entries (PR #362 / #364) that **do not exist in this repo's log** (`grep -c '^### 2026-05' ai-docs/learnings.md` → 0; the log spans 2026-07-13 onward) — an imported citation that was never checkable here, which is how it survived. Observed mechanism, for whoever probes next: the dispatch is **async** — it returned `Async agent launched successfully` with a task id, then delivered `PROBE_OK` via a later notification, so a probe expecting a blocking call-and-return could misread a successful launch as a failure.
