...
3. **Actively check the checklist:**
   ...
   - **No repo-internal references in planned doc-comment text** — when the design document contains inline rustdoc snippets (e.g. proposed `///` / `//!` text for an API), scan those snippets with Pattern A and Pattern B from [`ai-docs/doc-convention.md` § Self-sufficiency](../../ai-docs/doc-convention.md). Any match in a planned doc-comment block = `major`; matches in design-doc prose outside `///` / `//!` blocks are out of scope (the design doc is contributor surface, not rustdoc).
   - **No repo-internal inline `//` comments inside planned doc-comment fence content** — when the design document contains inline rustdoc snippets with code fences, apply the Family C §3 rule from [`ai-docs/doc-convention.md` § Self-sufficiency](../../ai-docs/doc-convention.md) to every inline `//` line inside those proposed fences. Any rule-(ii) match in planned fence content = `major`.
