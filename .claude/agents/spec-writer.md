...
```bash
printf '%s\n' "<draft questions> <draft spec body>" | grep -iE 'backward.compat|back.?compat|compat.shim|deprecat|keep.old|should.*panic|panic.or.return|for.users|existing.callers|would add.*dep|introduce.*as.*dep|pull in .* as.*dep|avoid.*as.*dep|not currently a depend|blocked.label|blocked.by.#|float.*(core|geom|sim|supercover|position|velocity)|(core|geom|sim|supercover).*(float|f32|f64)|is (committed|tracked|gitignored)|(are|were) (committed|tracked|gitignored)|commits (its|their|the) |still affects|is unfixed|no fix (released|is known)'
```
...
| `is committed`, `is tracked`, `is gitignored`, `commits its <X>` (any claim about this or a sibling repo's VCS state) | AGENTS.md § *Dependency Versions* AXIOM (VCS-state dimension): **name the file category first**, then run the command that can see it — tracked → `git ls-files <path>`; ignored + which rule → `git check-ignore -v <path>`; untracked-but-not-ignored → `git status --porcelain`; ignored included → `git status --porcelain --ignored`; on disk at all → `ls`/`find`. `find`/`ls` never prove tracked status; `git status` is blind to ignored files so its empty output never proves absence. Rewrite with the verified fact. **A retraction is an assertion too** — re-verify before writing one into the spec (this row exists because a `git status`-derived "0/0, did not reproduce" retraction reached `2026-07-16-render-backend-decision.spec.md:247`). |
| `still affects`, `is unfixed`, `no fix released`, `no fix is known` (any claim about an upstream issue's current state) | AGENTS.md § *Dependency Versions* AXIOM (upstream-issue dimension): run `gh issue view <N>` for state + comments. The issue *body* is frozen at filing time; the **closing comment** carries the resolution. If the user cited a URL with a `#fragment`, fetch **that anchor** — the fragment is the citation. Rewrite with the verified fact. |
